...
**Applies from:** tool version 1.1 (branch `hybrid-templates`), August 2026.
**Affects:** metadata generated for *new* configurations. Configurations
...
- they only produce values when a **scheduled predictor job** runs, and the
  results only become visible after the *next* analytics run — two moving
...
*hybrid*: one predictor computes the outlier threshold; everything else is
an indicator.
...
1. **Data element completeness (%)** — now
   ...
   across disaggregations, this counts *facilities reporting any value*
...
3. **Values that are outliers (%)** — numerator counts facilities whose
   ...
   *assessable* (have both a value and a threshold this month). Compares
...
2. **Outlier percentages only count assessable values.** The old
   configuration counted a facility that reported *nothing* as a
   ...
   *higher* (more honest) outlier percentages in months where some
...
3. **A facility's first months are blank, not 100% outlier.** The old
   ...
   compared as 0. The new indicators treat "no threshold" as *unassessable*
...
- **Monthly grain only.** `periodOffset` windows follow the *query's*
