...
window.importOutlier = async function () {
    ...
    if (addGroups) {
        try {
            ...
            importResult = await addToPdGroup(baseConfig.predictorGroupTreshold, splitPredictors(outlierConfig, outlierImport.predictors)["treshold"]);
            results.push(["Add to treshold predictor group", importResult["status"]]);
            importResult = await addToPdGroup(baseConfig.predictorGroupAnalysis, splitPredictors(outlierConfig, outlierImport.predictors)["analysis"]);
 ...
 */
function splitPredictors(outlierConfig, predictors) {
    let result = {
        "treshold": [],
        "analysis": []
    };
